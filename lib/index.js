...
var extractValue = function extractValue(x) {
	return t.isJSXExpressionContainer(x) ? x.expression : x;
};
...
var jsxExpression = function jsxExpression(x, callees) {
	return t.isJSXExpressionContainer(x) ? x.expression : transformJSX(x, callees);
};

var childrenToNodes = function childrenToNodes(children, callees) {
	return t.arrayExpression(children.map(function (c) {
		return jsxExpression(c, callees);
	}));
};
...
var makeHnode = function makeHnode(label, meta, children, callees) {
	...
	if (children.length > 0) {
		var childrenArg = childrenToNodes(children, callees);
